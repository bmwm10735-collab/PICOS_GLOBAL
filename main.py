...
BOT_TOKEN = "bot token"
ADMIN_ID = 6997272524
bot = TeleBot("8721519423:AAGlu1m48_13gzAzUHXOPt9vajgCsvp9Vz0")
...
class TelegramBotManager:
    ...
    def send_admin_panel(self):
        ...
        btn6 = types.InlineKeyboardButton(" تغيير الرسائل", callback_data="message")
        btn7 = types.InlineKeyboardButton(" حذف الحسابات⛔", callback_data="clear")
        ...
        bot.send_message(ADMIN_ID, " GLOBAL aMiNe ", reply_markup=markup)
    ...
    async def ChaT(self):
        ...
        while not self._stop_event.is_set():
            try:
                ...
                self.writer.write(GLobaL(T, self.key, self.iv))
                await self.writer.drain()
                await asyncio.sleep(0.5)
                ...
                while not self._stop_event.is_set():
                    try:
                        ...
                        if self.DaTa.hex().startswith("1200") and b"SecretCode" in self.DaTa:
                            ...
                            self.writer.write(RedZed_3alamyia_Chat(TarGeT, sQ, self.key, self.iv))
                            await self.writer.drain()
                            self.writer.write(RedZed_SendMsg(f"[[iِ][bِ][cِ][FFFFFF][B][B][B][B][B][C]{xMsGFixinG(message)}", TarGeT, self.bot_uid, self.key, self.iv))
                            await self.writer.drain()
                            
                            await asyncio.sleep(1)
                            
                            if self.writer2 and not self.writer2.is_closing():
                                self.writer2.write(RedZed_SendInv(self.bot_uid, TarGeT, self.key, self.iv))
                                await self.writer2.drain()
                            
                            self.writer2.write(Join_Sq(T, TarGeT, sQ, self.key, self.iv))
                            await self.writer2.drain()
                            
                            self.writer.write(quit_caht_redzed(TarGeT, self.key, self.iv))
                            await self.writer.drain()
                            ...
                            print(f"مع => {self.bot_uid} إلى => {TarGeT}")
...
def StarT_SerVer():
    ...
    TexT = f'[معلومات الهدف] > البوتات متصلة\n[حالة البوت] > [bold green]تم الاتصال بنجاح[/bold green]\n[تلجرام] > [bold blue]متصل ببوت الإدارة[/bold blue]'
    panel = Panel(Align.center(TexT), title="[bold yellow]FF - GLobaL (Stable)[/bold yellow]", border_style="bright_yellow", padding=(1, 2), expand=False)
    console.print(panel)
    ...
    try:
        bot.send_message(ADMIN_ID, " تم بدء تشغيل بوت فري فاير بنجاح!")
    except Exception as e:
